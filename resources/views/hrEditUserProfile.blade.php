...
<div class="breadcrumbs ace-save-state" id="breadcrumbs">
	...
	<div class="page-content">
		...
		<?php 
		$selectedDept=$users->department; 
		$selectedDeptManager=$users->reporting_to;
		?>
			...
			<div class="col-xs-12">	
				...
				<div class="form-group {{ $errors->has('Full Name') ? 'has-error' : '' }}">
					...
					<div class="form-group {{ $errors->has('manager_id') ? 'has-error' : '' }}">
						<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Reporting to : </label>

						<div class="col-sm-5">
						<select name="reporting_to" class="form-control">
							<option value="">Please select reporting Officer</option>
							@foreach($deptManagers as $deptManager)
							<option value="{{ $deptManager->id}}" {{ $selectedDeptManager == $deptManager->id ? 'selected="selected"' : '' }}>{{ $deptManager->name}}</option>
							@endforeach
						</select>							
						</div>
					...
					<div class="form-group {{ $errors->has('emp_ctc') ? 'has-error' : '' }}">
						...
						<span class="text-danger">{{ $errors->first('emp_ctc') }}</span>
					</div>
					
					<div class="form-group {{ $errors->has('is_dept_manager') ? 'has-error' : '' }}">
						<label class="col-sm-4 control-label no-padding-right">
							 Is Department Manager:
							 {!! Form::checkbox('is_dept_manager', 1, ($users->is_dept_manager) ? true : false, array('class'=>'autosize-transition form-control ace ace-switch ace-switch-5 form-control','id'=>'is_dept_manager')) !!} 
							 <span class="lbl middle"></span> 	
						</label>						
						<span class="text-danger">{{ $errors->first('is_dept_manager') }}</span>
					</div>
					
					<div class="form-group {{ $errors->has('is_hr_admin') ? 'has-error' : '' }}">
						<label class="col-sm-4 control-label no-padding-right">
							 Is HR Admin:
							 {!! Form::checkbox('is_hr_admin', 1, ($users->is_hr_admin) ? true : false, array('class'=>'autosize-transition form-control ace ace-switch ace-switch-5 form-control','id'=>'is_hr_admin')) !!} 
							 <span class="lbl middle"></span> 	
						</label>						
						<span class="text-danger">{{ $errors->first('is_hr_admin') }}</span>
					</div>
